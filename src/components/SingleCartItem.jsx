  ...
  return (
    <li className='border-b pt-2 pb-2'>
      <div className='flex'>
        ...
        <span className='border border-black py-2 px-5 mr-5'>{cartItem.quantity}</span>
        <span className='mt-2 w-18'>{(cartItemEl.price * cartItem.quantity).toFixed(2)} €</span>
        </div>
        <button onClick={() => {DeleteOfItem(), setTempCart([])}} className='flex w-1/4'><img className='h-8' src="/icons/Group1419.svg" alt="remove" /><span className='underline'>Remove</span></button>
        </div>
    </div>
    </li>
  )

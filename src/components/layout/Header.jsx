...
function Header() {
  ...
  return (
    ...
    >
      ...
      {windowWidth >= 1024 && isLoggedIn ? <div onClick={logoutUserFire} className="ml-20 cursor-pointer flex items-end gap-1 text-2xl hover:underline">
          <span className=""><i className="fa fa-user-circle text-green-400" aria-hidden="true"></i></span>
          <p>Logout</p>
          </div> : '' }
      {windowWidth >= 1024 && !isLoggedIn ? <div onClick={() => {navigate("/login")}} className="ml-20 cursor-pointer flex items-end gap-1 text-2xl hover:underline">

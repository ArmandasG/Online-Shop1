...
function Cart({ myCartNav, onClose }) {
  ...
  return (
    <div>
      {cartIsOpen && windowWidth >= 1024 ? (
        <div
          onClick={onClose}
          className={`absolute h-16 w-16 ${
            cartIsOpen && windowWidth < 1280 ? "right-[30%]" : "right-[20%]"
          }  top-[0%] border border-black bg-black text-white close-icon flex justify-center items-center appear-from-right cursor-pointer`}
        >
      ...
      <div
        className={`relative ${
          windowWidth < 1024 ? "cartOverlay" : "cartOverlayLg"
        }  pt-4`}
        id={myCartNav}
      >
        <div className="ml-8 mt-4 mr-8">
          ...
          <div className="lg:flex lg:justify-between lg:mt-8">
            ...
            <p className="text-end mt-8 lg:mt-0">
          ...
          <div
            onClick={onClose}
            className="flex justify-center cursor-pointer lg:hidden"
          >
            <i
